...
class FaceLexemeClip extends BaseClip {
	// to show captions
	static lexemes = [
	...
	static type = "faceLexeme";
	static id = ANIM.FACELEXEME != undefined ? ANIM.FACELEXEME: ANIM.clipTypes.length;
	static clipColor = "#57c1c1";
...
class FaceFACSClip extends BaseClip {
	static type = "faceFACS";
	static sides = ["LEFT", "RIGHT", "BOTH"];
	static id = ANIM.FACEFACS ? ANIM.FACEFACS: ANIM.clipTypes.length;
...
//FaceEmotionClip
class FaceEmotionClip extends BaseClip {
	static type = "faceEmotion";
	static emotions = ["HAPPINESS", "SADNESS", "SURPRISE", "FEAR","ANGER","DISGUST", "CONTEMPT"];
...
//FacePresetClip
class FacePresetClip extends BaseClip {
	static type = "facePreset";
	static facePreset = ["Yes/No-Question", "Negative", "WH-word Questions", "Topic", "RH-Questions", "Anger", "Happiness", "Fear", "Sadness", "Surprise", "Disgust", "Contempt"];
	static customPresets = {};
	
	static type = "facePreset";
	static id = ANIM.FACEPRESET ? ANIM.FACEPRESET: ANIM.clipTypes.length;
	static clipColor = "green";
...
/*----------------------------------Gaze Behaviour-----------------------------------*/
//GazeClip
class GazeClip extends BaseClip {
	static type = "gaze";
	static influences = ["Eyes", "Head", "Neck"];
	static directions = ["", "Up right", "Up left", "Down right", "Down left", "Right", "Left", "Up", "Down"];
	static targets = ["Up right", "Up left", "Down right", "Down left", "Right", "Left", "Up", "Down", "Front"];
	
	static id = ANIM.GAZE ? ANIM.GAZE: ANIM.clipTypes.length;
	static clipColor = "#578cc1"//"#8275bd";
...
/*----------------------------------Head Behaviour-----------------------------------*/
//HeadClip
class HeadClip extends BaseClip {
	static type = "head";
	static lexemes = ["Nod", "Shake", "Rotate left", "Rotate right", "Tilt", "Tilt left", "Tilt right", "Tilt forward", "Tilt backward", "Forward", "Backward"];
	static id = ANIM.HEAD ? ANIM.HEAD: ANIM.clipTypes.length;
	static clipColor = "#5772c1";
...
//ShoulderClip
class ShoulderClip extends BaseClip {
	static type = "gesture";
	static hands = ["Left", "Right", "Both"];
	static movements = ["Raise", "Hunch"];
	static id = ANIM.SHOULDER ? ANIM.SHOULDER: ANIM.clipTypes.length;
	static clipColor = "#c157c1";
...
//BodyMovementClip
class BodyMovementClip extends BaseClip {
	static type = "gesture";
	static movements = ["Tilt forward", "Tilt backward", "Tilt left", "Tilt right", "Rotate left", "Rotate right"];
	static hands = ["Left", "Right", "Both"];
	static id = ANIM.BODYMOVEMENT ? ANIM.BODYMOVEMENT: 8;
	static clipColor = "#c18257";
...
// helper function to add a widget to add directions
function makeDirectionsInput ( id, panel, targetArray, callback, options= {} ) {

	const directions = options.directions ?? ( 0x01 | 0x02 | 0x04 ); // horizontal | vertical | depth

	panel.sameLine();
	let tags = panel.addTags( id, "", (v,e)=>{
		if ( e instanceof KeyboardEvent ){
			let srcLength = v.length;
			let itemStart = v.pop()[0].toUpperCase();					
			if ( directions & 0x01 ){
				if ( itemStart == "L" ){ v.push("Left"); }
				if ( itemStart == "R" ){ v.push("Right"); }
			}
			if ( directions & 0x02 ){
				if ( itemStart == "U" ){ v.push("Up"); }
				if ( itemStart == "D" ){ v.push("Down"); }
			}
			if ( directions & 0x04 ){
				if ( itemStart == "I" ){ v.push("Inwards"); }
				if ( itemStart == "O" ){ v.push("Outwards"); }
			}
			tags.generateTags( v );

			// invalid tag that was not added. Nothing to update
			if ( srcLength != v.length ){
				return;
			}
		} 
	...
	panel.addButton( "DirectionsPopover", "", ( v, event ) => { 

		const popoverArea = new LX.Area( { className: "", skipAppend: true } );
		const popoverPanel = popoverArea.addPanel( );

		const gridPopover = LX.makeContainer( [ "100%", "auto" ], "p-1 whitespace-pre-wrap", 
			"", 
			popoverPanel.root, 
			{ display: "grid", gridTemplateColumns: "" } 
		);

		const makeButton = (name, tag) => {
			let b = new LX.Button("", name, (v,e)=>{ 
		...
		// first row
		if ( directions & 0x01 ){ makeButton( "L", "Left" ); gridPopover.style.gridTemplateColumns += " auto"; }
		if ( directions & 0x02 ){ makeButton( "U", "Up" ); gridPopover.style.gridTemplateColumns += " auto"; }
		if ( directions & 0x04 ){ makeButton( "I", "Inwards" ); gridPopover.style.gridTemplateColumns += " auto"; }

		// second row
		if ( directions & 0x01 ){ makeButton( "R", "Right" ); }
		if ( directions & 0x02 ){ makeButton( "D", "Down" ); }
		if ( directions & 0x04 ){ makeButton( "O", "Outwards" ); }

		new LX.Popover( event.target, [ popoverArea ], { side: "bottom" } );
	}, { icon: "Move", height: "100%", width: "10%", buttonClass: "", className: "self-center", hideName: true } );
	panel.endLine();
}

/**
 * Transforms strings of bml directions "ulr" to an Array of readable strings ["up", "left", "right"] 
 * @param {string} str source string to parse of the style "ulr" meaning up left right 
 * @param {integer} validDirections flags: 1 left-right, 2 up-down, 4 in-out 
 * @param {array} targetArray where to put resulting strings 
 * @returns 
 */
function strToDirection ( str, validDirections = 0x01 | 0x02 | 0x04, targetArray = [] ) {
	for(let i = 0; i < str.length; i++) {
		switch(	str[i].toLowerCase() ) {
			case "l":
				if( validDirections & 0x01 ){ targetArray.push( "Left" ); }
				break;
			case "r":
				if( validDirections & 0x01 ){ targetArray.push( "Right" ); }
				break;
			case "u":
				if( validDirections & 0x02 ){ targetArray.push( "Up" ); }
				break;
			case "d":
				if( validDirections & 0x02 ){ targetArray.push( "Down" ); }
				break;
			case "i":
				if( validDirections & 0x04 ){ targetArray.push( "Inwards" ); }
				break;
			case "o":
				if( validDirections & 0x04 ){ targetArray.push( "Outwards" ); }
				break;
		}
	}
...
//ArmLocationClip
class ArmLocationClip extends BaseClip {
	static type = "gesture";
	static locations = ["Head", "Head top", "Forehead", "Nose", "Below nose", "Chin", "Under chin", "Mouth", "Earlobe", "Earlobe Right", "Earlobe Left", "Ear", "Ear Right", "Ear Left", "Cheek", "Cheek Right", "Cheek Left", 
	...
	static sides = { "Right": "rr", "Slightly right": "r", "Left": "ll", "Slightly left": "l"};
	static hands = ["Left", "Right", "Both"];
	static directions = ["Up", "Down", "Left", "Right", "In", "Out",
	...
	static fingers = ["","Thumb", "Index", "Middle", "Ring", "Pinky"];
	static hand_locations = ["","Tip", "Pad", "Mid", "Base", "Thumb ball", "Hand", "Wrist"];
	static hand_sides = ["", "Right", "Left", "Ulnar", "Radial", "Front", "Back", "Palmar"];
	
	static id = ANIM.ARMLOCATION ? ANIM.ARMLOCATION: ANIM.clipTypes.length;
	static clipColor = "#c157a7";
	...
	toJSON()
	{
		...
		for(let i in this.properties)
		{
			if( i == "side" || i == "secondSide" ) 
				...
				json[i] = directionsToStr( this.properties[i] );
			}
			else if(i == "srcFinger") {
	...
	showInfo(panel, callback)
	{
		...
		makeDirectionsInput( "Displace", panel, this.properties.displace, callback, { directions: 0x07, title: "Location will be offseted into that direction" }); // LRUDIO


		if(this.properties.displace.length) {
...
//PalmOrientationClip
class PalmOrientationClip extends BaseClip {
	static type = "gesture";
	static hands = ["Left", "Right", "Both"];
	static directions = ["Up", "Down", "Left", "Right", "Up Left", "Up Right", "Down Left", "Down Right"];
	
	static id = ANIM.PALMORIENTATION ? ANIM.PALMORIENTATION: ANIM.clipTypes.length;
	static clipColor = "#a7c157";
...
//HandOrientationClip
class HandOrientationClip extends BaseClip {
	static type = "gesture";
	static hands = ["Left", "Right", "Both"];
	static directions = ["Up", "Down", "Left", "Right", "In", "Out",
									"Up Left", "Up Right", "Up In", "Up Out",
	...
	static id = ANIM.HANDORIENTATION ? ANIM.HANDORIENTATION: ANIM.clipTypes.length;
	static clipColor = "#8cc157";
	...
	toJSON()
	{
		...
		for(var i in this.properties)
		{
			for(var i in this.properties)
			{
				if ( i == "extfidir" || i == "secondExtfidir" ) {
					json[i] = directionsToStr( this.properties[i] );
				}
				else 
					json[i] = typeof(this.properties[i]) == 'string' ? this.properties[i].replaceAll(" ", "_").toUpperCase() : this.properties[i];
...
//HandshapeClip
class HandshapeClip extends BaseClip {
	static type = "gesture";
	static handshapes = ["Fist", "Finger 2", "Finger 23", "Finger 23 spread", "Finger 2345", "Flat", "Pinch 12", "Pinch 12 open", "Pinch all", "Cee all", "Cee 12", "Cee 12 open"];
	static thumbshapes = ["Default", "Out", "Opposed", "Across"];
	static bendstates = ["Straight", "Half bent", "Bent", "Round", "Hooked", "Double bent", "Double hooked"];
	static hands = ["Left", "Right", "Both"];
	static fingers = ["", "Index", "Middle", "Ring", "Pinky"];
	
	static id = ANIM.HANDSHAPE ? ANIM.HANDSHAPE: ANIM.clipTypes.length;
	static clipColor = "#57c172";
	...
	constructor(o)
	{
		...
		this.properties = {
			...
			bend2:null, //Explicit individual digit bending specifications, with the same permitted values as the mainbend
			bend3:null,
			bend4:null,
			bend5: null,
	...
	showInfo(panel, callback)
	{
		...
		addBendSplay("bend2", "Bend Index", false, { title: "Overrides any bending defined explicitly or implicitly by earlier attributes to this finger" });
		addBendSplay("bend3", "Bend Middle", false, { title: "Overrides any bending defined explicitly or implicitly by earlier attributes to this finger" });
		addBendSplay("bend4", "Bend Ring", false, { title: "Overrides any bending defined explicitly or implicitly by earlier attributes to this finger" });
		addBendSplay("bend5", "Bend Pinky", false, { title: "Overrides any bending defined explicitly or implicitly by earlier attributes to this finger" });
		...
		addBendSplay("splay5", "Splay Pinky", true,{ title: "Overrides any splaying defined explicitly or implicitly by earlier attributes to this finger" });
	}
}
...
class HandConstellationClip extends BaseClip {
	static type = "gesture";
	static hands = ["Left", "Right", "Both", "Dominant", "Non dominant"];
	static arm_locations = ["Forearm", "Elbow", "Upper arm"];
	static hand_locations = ["","Tip", "Pad", "Mid", "Base", "Thumb ball", "Hand", "Wrist"];
	static hand_sides = ["", "Right", "Left", "Ulnar", "Radial", "Back", "Palmar"];
	static fingers = ["","Thumb", "Index", "Middle", "Ring", "Pinky"];
	static directions = ["Up", "Down", "Left", "Right", "In", "Out",
										"Up Left", "Up Right", "Up In", "Up Out",
										"Left In", "Left Out", "Right In", "Right Out",								
	...
	static id = ANIM.HANDCONSTELLATION ? ANIM.HANDCONSTELLATION: ANIM.clipTypes.length;
	static clipColor = "#c1578c";
	...
	showInfo(panel, callback)
	{
		...
		// Displacement property
		makeDirectionsInput( "Distance direction", panel, this.properties.distanceDirection, callback, { directions: 0x07 }); // LRUDIO
...
//DirectedMotionClip
class DirectedMotionClip extends BaseClip {
	static type = "gesture";
	static hands = ["Left", "Right", "Both"];
	static directions = ["Up", "Down", "Left", "Right", "In", "Out",
									"Up Left", "Up Right", "Up In", "Up Out",
									...
									"Down In Left", "Down In Right", "Down Out Left", "Down Out Right",
									"Up In Left", "Up In Right", "Up Out Left", "Up Out Right"]
	static second_directions = ["Up", "Down", "Left", "Right","Up Left", "Up Right", "Down Left", "Down Right"];
	
	static id = ANIM.DIRECTEDMOTION ? ANIM.DIRECTEDMOTION: ANIM.clipTypes.length;
	static clipColor = "#c15772";
	...
	toJSON()
	{
		...
		for(let i in this.properties)
		{
			if ( i == "direction" || i == "secondDirection" || i == "curve" || i == "secondCurve" || i == "zigzag" ) {
				json[i] = directionsToStr( this.properties[i] );
			}
			else 
				json[i] = typeof(this.properties[i]) == 'string' ? this.properties[i].replaceAll(" ", "_").toUpperCase() : this.properties[i];
	...
	showInfo(panel, callback)
	{
		...
		// Displacement property
		makeDirectionsInput( "Second direction", panel, this.properties.secondDirection, callback, { directions: 0x07 }); // LRUDIO
		...
		makeDirectionsInput( "Curve direction", panel, this.properties.curve, callback, { directions: 0x07 }); // LRUDIO
		
		makeDirectionsInput( "Second curve direction", panel, this.properties.secondCurve, callback, { directions: 0x07 }); // LRUDIO
		...
		panel.addCheckbox("Apply zig-zag", this.zigzagEnabler, (v, e, name) =>
		{
			...
			suboptions: (p) =>{
				...
				makeDirectionsInput( "Zig zag direction", p, this.properties.zigzag, callback, { directions: 0x07 }); // LRUDIO
...
//CircularMotionClip
class CircularMotionClip extends BaseClip {
	static type = "gesture";
	static hands = ["Left", "Right", "Both"];
	static directions = ["Up", "Down", "Left", "Right", "In", "Out",
									"Up Left", "Up Right", "Up In", "Up Out",
	...
	static id = ANIM.CIRCULARMOTION ? ANIM.CIRCULARMOTION: ANIM.clipTypes.length;
	static clipColor = "#c15757";
	...
	toJSON()
	{
		...
		for(let i in this.properties)
		{
			if ( i == "direction" || i == "secondDirection" || i == "ellipseAxisDirection"  || i == "zigzag" ) {
				json[i] = directionsToStr( this.properties[i] );
			}
			else 
				json[i] = typeof(this.properties[i]) == 'string' ? this.properties[i].replaceAll(" ", "_").toUpperCase() : this.properties[i];
	...
	showInfo(panel, callback)
	{
		...
		makeDirectionsInput( "Ellipse axis direction", panel, this.properties.ellipseAxisDirection, callback, { directions: 0x07, title: "Direction of the axis of the ellipse. Defines the plane orientation." }); // LRUDIO
		...
		panel.addCheckbox("Apply zig-zag", this.zigzag, (v, e, name) =>
		{
			...
			className: "contrast", label: "",
			suboptions: (p) =>{
				makeDirectionsInput( "Zig zag direction", p, this.properties.zigzag, callback, { directions: 0x07, title: "Direction of the axis of the ellipse. Defines the plane orientation." }); // LRUDIO
		
				p.addNumber("Zig zag amplitude (m)", this.properties.zigzagSize ?? 0.01, (v, e, name) =>
				{
					this.properties.zigzagSize = v;
					if(callback)
						callback();
				...
				p.addNumber("Oscillations per second", this.properties.zigzagSpeed ?? 2, (v, e, name) =>
				{
					this.properties.zigzagSpeed = v;
					if(callback)
						callback();
...
//WristMotionClip
class WristMotionClip extends BaseClip {
	static type = "gesture";
	static hands = ["Left", "Right", "Both"];
	static modes = ["Nod", "Swing", "Twist", "Stir CW", "Stir CCW", "All"];
	
	static id = ANIM.WRISTMOTION ? ANIM.WRISTMOTION: ANIM.clipTypes.length;
	static clipColor = "#72c157";
...
class FingerplayMotionClip extends BaseClip {
	static type = "gesture";
	static hands = ["Left", "Right", "Both"];
	static fingers = ["Thumb", "Index", "Middle", "Ring", "Pinky"];
	
	static id = ANIM.FINGERPLAYMOTION ? ANIM.FINGERPLAYMOTION: ANIM.clipTypes.length;
	static clipColor = "#57c157";
...
//MouthingClip
class MouthingClip extends BaseClip {
	static type = "speech";
	static id = ANIM.FINGERPLAYMOTION ? ANIM.FINGERPLAYMOTION: ANIM.clipTypes.length;
	static clipColor = "#57c1a7";
...
class SuperClip extends BaseClip {
	static type = "super";
	static id = ANIM.SUPERCLIP ? ANIM.SUPERCLIP: ANIM.clipTypes.length;
	static clipColor = "#9398AB";
	static clipTimeModes = { LOCAL: 0, GLOBAL: 1 };
	 ...
	 */
	changeTimeModeOfClip( clip, targetMode ){
