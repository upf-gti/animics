...
// asset view was not displaying contents
LX.AssetView.prototype._createContentPanel = function( area ) {
    ...
    {
        this.toolsPanel = area.addPanel({ className: 'flex flex-col overflow-hidden', height:"auto" }); // <--- added height:auto
    ...
    this.toolsPanel.refresh = () => {
        ...
        this.toolsPanel.endLine(); // <--- added this

        if( !this.skipBrowser )
        {
            this.toolsPanel.sameLine(); // <--- added this
            ...
            this.toolsPanel.endLine(); // <--- moved this inside the if 
        }
    };
